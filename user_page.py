...
import food_api_service , favorites_service , gui
...
class User_Page(QWidget):
    def __init__(self, stacked_widget):
        ...
        self.add_item_to_favorites = QPushButton("Add favorites", self)
        ...
        self.add_item_to_favorites.clicked.connect(self.clicked_add_favorites_button)
        ...
        if not product:
            ...
            if isinstance(data,dict):
                if "error" in data:
                    ...
                    doctor_respond=self.parse_ai_respond(ai_result)
                    ...
                    self.current_data = {
                        ...
                        'ai_comment': doctor_respond.get('COMMENT', 'No comment'),
                        'ai_suggestion': doctor_respond.get('SUGGESTION', 'No suggestion'),
                        'ai_category': doctor_respond.get('CATEGORY', 'Unknown'),
                        'ai_score': doctor_respond.get('SCORE', 'N/A')
                    }
    ...
    def parse_ai_respond(self,ai_data):
        result_dic = {}
        lines = ai_data.split('\n')
    ...
    def clicked_add_favorites_button(self):
        try:
            if not self.current_data:
                self.result_label.setText("❌ No product data available!")
                return False
        
            name=self.current_data["name"]
            barcode=self.current_data["barcode"]
            brand=self.current_data["brand"]
            ingredients=self.current_data["ingredients"]
            ai_comment=self.current_data["ai_comment"]
            ai_suggestion=self.current_data["ai_suggestion"]
            ai_category=self.current_data["ai_category"]
            ai_score=self.current_data["ai_score"]

            product_id=favorites_service.add_product(
                name,
                barcode,
                brand,
                ingredients
            ...
            if not product_id:
                self.result_label.setText("❌ Error adding product!")
                return False
            
            ai_analyz_id=favorites_service.add_ai_analyz(
                product_id,
                ai_score,
                ai_comment,
                ai_category,
            ...
            user_id=self.stacked_widget.current_user_id

            success=favorites_service.add_favorites(user_id,product_id,ai_analyz_id)

            if success:
                self.label1.setText("Product added to favorites successfully")
                return True
            else:
                self.label1.setText("Product could not add to favorites successfully")
                return 

        
        except Exception as e:
            self.result_label.setText("Error! product could not added in favorites")
            return False
    ...
    def initUI(self):
            ...
            hbox.addWidget(self.add_item_to_favorites)
            vbox.addLayout(hbox)
